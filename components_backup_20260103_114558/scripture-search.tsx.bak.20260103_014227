      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="font-playfair text-3xl md:text-4xl font-bold text-foreground mb-4">
            {t("scriptureSearch.title")}
          </h2>
          <p className="text-lg text-muted-foreground">{t("scriptureSearch.subtitle")}</p>
        </div>

        <Card className="max-w-2xl mx-auto">
          <CardHeader>
            <CardTitle className="text-center font-playfair">{t("scriptureSearch.cardTitle")}</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <Button asChild className="w-full mt-4" size="lg">
              <Link href={`/versiculos?busqueda=${encodeURIComponent(searchTerm)}`}>
                {t("scriptureSearch.searchButton")}
              </Link>
            </Button>
          </CardContent>
        </Card>
